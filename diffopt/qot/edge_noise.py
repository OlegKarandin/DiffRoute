...
Reuses the per-span ASE term shape from the analytical GN model (the h*nu
physical constant is dropped — only relative magnitude across edges matters,
since the output is median-normalized below).
"""
...
def compute_edge_ase_noise(topology: Topology) -> torch.Tensor:
    ...
    for edge in topology.undirected_edges:
        loss_coef_db_km = topology.get_fiber_type(edge.fiber_type).loss_coef_db_per_km
        edge_noise = 0.0
        for span_km, nf_db in zip(edge.span_lengths_km, edge.amplifier_nf_db):
            span_loss_lin = 10.0 ** (loss_coef_db_km * span_km / 10.0)
